...
# from s2s_ft.lsh_attention import LSHSelfAttention
from s2s_ft.sinkhorn import SinkhornSelfAttention
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, encoder_hidden_states=None, split_lengths=None):
        ...
        if encoder_hidden_states is not None:
            mixed_key_layer = torch.cat((self.key(encoder_hidden_states), self.key(hidden_states)), dim=1)
            mixed_value_layer = torch.cat((self.value(encoder_hidden_states), self.value(hidden_states)), dim=1)
        else:
        ...
        if split_lengths:
            ...
            query_parts = torch.split(mixed_query_layer, split_lengths[1:], dim=1)
            ...
            sum_length = 0
            cur_idx = 0
            for (_key, _value, part_length) in zip(key_parts, value_parts, split_lengths):
                query = query_parts[cur_idx - 1]
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.sinkhorn = SinkhornSelfAttention()
        
        # self.sinkhorn.q_proj.weight.data = copy.deepcopy(self.self.query.weight.data)
        # self.sinkhorn.k_proj.weight.data = copy.deepcopy(self.self.key.weight.data)
        # self.sinkhorn.v_proj.weight.data = copy.deepcopy(self.self.value.weight.data)
        # self.sinkhorn.q_proj.bias.data = copy.deepcopy(self.self.query.bias.data)
        # self.sinkhorn.k_proj.bias.data = copy.deepcopy(self.self.key.bias.data)
        # self.sinkhorn.v_proj.bias.data = copy.deepcopy(self.self.value.bias.data)

        self.output = BertSelfOutput(config)
        # self.sinkhorn.out_proj.weight.data = copy.deepcopy(self.output.dense.weight.data)
        # self.sinkhorn.out_proj.bias.data = copy.deepcopy(self.output.dense.bias.data)


    def forward(self, hidden_states, attention_mask=None, key_padding_mask=None,
                encoder_hidden_states=None, split_lengths=None, layer_idx=None):
        if (encoder_hidden_states is not None) or (layer_idx is not None and layer_idx % 4 == 3):
            # decoder attention or encoder attention (last 2 layer)
            self_outputs = self.self(
                hidden_states, attention_mask=attention_mask,
                encoder_hidden_states=encoder_hidden_states, split_lengths=split_lengths)
            attention_output = self.output(self_outputs[0], hidden_states)
            outputs = (attention_output,) + self_outputs[1:]  # add attentions if we output then
        else:
            assert layer_idx in [0, 1, 2, 4, 5, 6, 8, 9, 10]
            # sinkhorn attention
            sinkhorn_outputs = self.sinkhorn(query=hidden_states, key_padding_mask=key_padding_mask)
            attention_output = self.output(sinkhorn_outputs[0], hidden_states)
            outputs = (attention_output,) + sinkhorn_outputs[1:]  # add attentions if we output then
        return outputs
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, key_padding_mask=None,
        ...
        self_attention_outputs = self.attention(
            hidden_states, attention_mask, key_padding_mask, 
            encoder_hidden_states=encoder_hidden_states, split_lengths=split_lengths, layer_idx=layer_idx)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, key_padding_mask=None, encoder_hidden_states=None, split_lengths=None):
        all_hidden_states = ()
        all_attentions = ()
        for i, layer_module in enumerate(self.layer):
            ...
            cur_encoder_hidden_states = encoder_hidden_states[i] if encoder_hidden_states is not None else None
            layer_outputs = layer_module(hidden_states, attention_mask, key_padding_mask,
                                        encoder_hidden_states=cur_encoder_hidden_states, split_lengths=split_lengths, layer_idx=i)
...
class BertModel(BertPreTrainedForSeq2SeqModel):
    ...
    # for sinkhorn attention
    def get_key_padding_mask(self, input_ids, pad_idx=0):
        return input_ids == pad_idx # for BERT tokenizer

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                position_ids=None, inputs_embeds=None, encoder_hidden_states=None, split_lengths=None, is_decoder=False):
        ...
        if is_decoder == True and encoder_hidden_states is None:
            raise ValueError("You should input encoder_hidden_states if you want to use BertModel as a decoder")
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0 # for self attention
        
        key_padding_mask = self.get_key_padding_mask(input_ids) # for sinkhorn attention

        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds)
        encoder_outputs = self.encoder(
            embedding_output, attention_mask=extended_attention_mask, key_padding_mask=key_padding_mask,
            encoder_hidden_states=encoder_hidden_states, split_lengths=split_lengths)
...
class BertForSequenceToSequence(BertPreTrainedForSeq2SeqModel):
    ...
    def forward(self, source_ids, target_ids, pseudo_ids, num_source_tokens, num_target_tokens, target_span_ids=None):
        ...
        encoder_outputs = self.bert(
            input_ids[:, :source_len], attention_mask=attention_mask[:, :source_len, :source_len], 
            token_type_ids=token_type_ids[:, :source_len], position_ids=position_ids[:, :source_len])

        last_states, all_states = encoder_outputs[0], encoder_outputs[1]
        all_states = [x for x in all_states] # [embedding output, 12 layer hidden states]
        assert len(all_states) == 13

        # decoder part
        decoder_outputs = self.bert(
            input_ids[:, source_len:], attention_mask=attention_mask,
            token_type_ids=token_type_ids[:, source_len:], position_ids=position_ids[:, source_len:],
            encoder_hidden_states=all_states, split_lengths=split_lengths, is_decoder=True)
        
        sequence_output = decoder_outputs[0]
        # pseudo_sequence_output = sequence_output[:, source_len + target_len:, ]
        pseudo_sequence_output = sequence_output[:, target_len:, ]
