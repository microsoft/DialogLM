...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, key_padding_mask=None, history_states=None,
                mask_qkv=None, seg_ids=None, key_history=None, value_history=None, layer_idx=None):
        if (history_states is not None) or (layer_idx is not None and layer_idx % 4 == 3):
            # decode attention or last 2 layer of encod attention
            ...
            sinkhorn_output = self.sinkhorn(input_tensor, key_padding_mask)[0]
            # attention_output = self_output[0]
            attention_output = self.output(sinkhorn_output, input_tensor)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, key_padding_mask=None, history_states=None,
                mask_qkv=None, seg_ids=None, key_history=None, value_history=None, layer_idx=None):
        attention_output = self.attention(
            hidden_states, attention_mask, key_padding_mask=key_padding_mask, history_states=history_states,
            mask_qkv=mask_qkv, seg_ids=seg_ids, key_history=key_history, value_history=value_history, layer_idx=layer_idx)
        if self.ffn_type:
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, key_padding_mask=None, output_all_encoded_layers=True, prev_embedding=None,
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                ...
                hidden_states = layer_module(
                    hidden_states, attention_mask, key_padding_mask=key_padding_mask, mask_qkv=mask_qkv, seg_ids=seg_ids,
                    key_history=set_key, value_history=set_value, layer_idx=i)
                if output_all_encoded_layers:
                    all_encoder_layers.append(hidden_states)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, key_padding_mask=None, output_all_encoded_layers=True,
        ...
        encoded_layers = self.encoder(embedding_output, extended_attention_mask, key_padding_mask,
...
class BertModelIncr(BertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True,
        ...
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)
        key_padding_mask = self.get_key_padding_mask(input_ids)
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        # encoder part
        prev_embedding, prev_encoded_layers, _ = self.bert(input_ids, token_type_ids[:, :input_length], 
                                                    position_ids[:, :input_length], attention_mask[:, :input_length, :input_length],
                                                    output_all_encoded_layers=True, mask_qkv=mask_qkv)
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        while next_pos < output_length:
            ...
            if len(total_scores) == 0:
                ...
                ### back_ptrs = torch.div(k_ids, K) # for pytorch <= 1.4.0
                back_ptrs = k_ids // K # for pytorch 1.7.0
            ...
            if self.pos_shift:
                ...
                if is_first:
                    prev_encoded_layers = [first_expand(x) for x in prev_encoded_layers]
                else:
                    prev_encoded_layers = [torch.cat((x[0], x[1][:, :-1, :]), dim=1)
                                           for x in zip(prev_encoded_layers, new_encoded_layers)]
                    prev_encoded_layers = [select_beam_items(
