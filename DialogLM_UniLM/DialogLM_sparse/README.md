# DialogLM-sparse (with sparse attention)
...
Beisdes, please install FairSeq (`pip install fairseq`).

## Pre-trained Models

Pre-trained Model (**DialogLM-sparse**) can be downloaded from [here](https://drive.google.com/file/d/1E6z_R9ne2Od-jagzhGv5Ds-tMI_etwbx/view?usp=sharing).
...
## Example: AMI with DialogLM-sparse
...
python -m torch.distributed.launch --nproc_per_node=8 run_seq2seq.py \
  ...
  --do_lower_case --fp16 --fp16_opt_level O2 --max_source_seq_length 8192 --max_target_seq_length 512 \
  --per_gpu_train_batch_size 1 --gradient_accumulation_steps 8 \
  --learning_rate 2e-5 --num_warmup_steps 500 --num_training_steps 5000 --save_steps 500 --cache_dir ${CACHE_DIR}
```
...
python decode_seq2seq.py \
  ...
  --model_path ${MODEL_PATH} --max_seq_length 8704 --max_tgt_length 512 --batch_size 2 --beam_size 8 \
...
## Example: TVMegaSite with DialogLM-sparse
...
python decode_seq2seq.py \
  --fp16 --model_type unilm --tokenizer_name unilm1.2-base-uncased --input_file ${INPUT_JSON} --split $SPLIT --do_lower_case \
  --model_path ${MODEL_PATH} --max_seq_length 8552 --max_tgt_length 360 --batch_size 4 --beam_size 6 \
